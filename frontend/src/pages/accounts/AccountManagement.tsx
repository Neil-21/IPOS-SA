...
function formToDiscountPlan(form: FormData): DiscountPlan {
  if (form.discountType === "fixed" || form.discountType === "none")
    return {
      type: "fixed",
      rate: form.discountType === "none" ? 0 : parseFloat(form.fixedRate) / 100,
    };
  return {
    type: "flexible",
    tiers: [
      {
        minValue: 0,
        maxValue: parseFloat(form.tier1Max),
        rate: parseFloat(form.tier1Rate) / 100,
      },
      {
        minValue: parseFloat(form.tier1Max),
        maxValue: parseFloat(form.tier2Max),
        rate: parseFloat(form.tier2Rate) / 100,
      },
      {
        minValue: parseFloat(form.tier2Max),
        maxValue: null,
        rate: parseFloat(form.tier3Rate) / 100,
      },
    ],
  };
}
...
function merchantToForm(m: Merchant): FormData {
  const base: FormData = {
    ...
    fax: m.fax ?? "",
    email: m.email,
    creditLimit: String(m.creditLimit),
    loginUsername: m.loginUsername ?? "",
    loginPassword: "",
    discountType:
      m.discountPlan.type === "flexible"
        ? "flexible"
        : m.discountPlan.rate === 0
          ? "none"
          : "fixed",
    fixedRate:
      m.discountPlan.type === "fixed" ? String(m.discountPlan.rate * 100) : "2",
    tier1Max: "1000",
    tier1Rate: "0",
    tier2Max: "2000",
    tier2Rate: "1",
...
export function AccountManagementPage() {
  ...
  // ✅ STEP 3: Clear error when modal closes
  const closeModal = () => {
    setModalOpen(false);
  ...
  const openEdit = (m: Merchant) => {
    setEditMerchant(m);
    setForm(merchantToForm(m));
    setModalOpen(true);
  };

  // ✅ STEP 2: Updated handleSave with validation
  const handleSave = async () => {
    const missing: string[] = [];
    if (!form.companyName.trim()) missing.push("Company Name");
    if (!form.contactName.trim()) missing.push("Contact Name");
    if (!form.email.trim()) missing.push("Email");
    if (!editMerchant && !form.loginPassword.trim()) missing.push("Password");
    if (!form.creditLimit.trim()) missing.push("Credit Limit");
  ...
  const changeStatus = async (id: string, newStatus: AccountStatus) => {
    ...
    if (!merchant) return;
    if (
      merchant.accountStatus === "in_default" &&
      newStatus === "normal" &&
      !isManager
    ) {
      alert(
        "Only the Director of Operations (Manager) can restore an In Default account.",
      );
      return;
    }
    try {
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      alert(
        `Failed to record payment: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
  };
  ...
  const columns: TableColumn<Merchant>[] = [
    {
      ...
      width: "110px",
      render: (r) => (
        <span className="mono" style={{ fontWeight: 700, fontSize: "12px" }}>
          {r.iposAccount}
        </span>
      ),
    },
    ...
    { key: "contactName", header: "Contact" },
    {
      ...
      align: "right",
      render: (r) => (
        <span className="mono">£{r.creditLimit.toLocaleString()}</span>
      ),
    },
    {
      key: "currentBalance",
      ...
      render: (r) => (
        <span style={{ fontSize: "12px", color: "var(--color-text-2)" }}>
          {r.discountPlan.type === "fixed"
            ? r.discountPlan.rate === 0
              ? "None"
              : `Fixed ${(r.discountPlan.rate * 100).toFixed(0)}%`
            : "Flexible (tiered)"}
        </span>
      ),
    },
    {
      key: "accountStatus",
      header: "Status",
      render: (r) => <AccountStatusBadge status={r.accountStatus} />,
    },
    {
      key: "actions",
      header: "",
        ...
        <div style={{ display: "flex", gap: "5px" }}>
          <Button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setViewId(row.id);
              setPayAmount(row.currentBalance.toFixed(2));
            }}
          >
            View
          </Button>
  ...
  return (
    <Page
      ...
      subtitle={`${merchants.length} registered merchants`}
      actions={
    ...
    >
      ...
      >
        {[
          ...
          {
            label: "Normal",
            count: merchants.filter((m) => m.accountStatus === "normal").length,
            color: "var(--color-success)",
            filter: "normal",
          },
          {
            label: "Suspended",
            count: merchants.filter((m) => m.accountStatus === "suspended")
              .length,
          ...
          {
            label: "In Default",
            count: merchants.filter((m) => m.accountStatus === "in_default")
              .length,
          ...
          <button
            key={label}
            onClick={() => setStatusFilter(filter as AccountStatus | "all")}
            style={{
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {liveView?.accountStatus === "suspended" && (
              <Button
                ...
                onClick={() => changeStatus(liveView!.id, "normal")}
              >
                Restore to Normal
              </Button>
            )}
            ...
            {liveView?.accountStatus === "normal" && (
              <Button
                ...
                onClick={() => changeStatus(liveView!.id, "suspended")}
              >
                Suspend
              </Button>
            )}
            {liveView?.accountStatus === "suspended" && (
              <Button
                ...
                onClick={() => changeStatus(liveView!.id, "in_default")}
              >
                Mark In Default
              </Button>
            )}
            {liveView?.discountPlan.type !== "fixed" ||
            (liveView.discountPlan.type === "fixed" &&
              liveView.discountPlan.rate > 0) ? (
              <Button
                variant="ghost"
                size="sm"
                onClick={async () => {
                  await updateMerchant(liveView!.id, {
                    discountPlan: { type: "fixed", rate: 0 },
                  });
                }}
              >
                Remove Discount Plan
              </Button>
            ) : null}
            <Button
              size="sm"
        ...
        {liveView && (
          ...
          >
            ...
            >
              <div>
                ...
                >
                  ...
                  {liveView.loginUsername && (
                    <span style={{ marginLeft: 12 }}>
                      Login: <strong>{liveView.loginUsername}</strong>
                    </span>
                  )}
                </p>
                <p style={{ fontSize: "13px", color: "var(--color-text-2)" }}>
                  {liveView.contactName} · {liveView.address}, {liveView.city}{" "}
                  {liveView.postcode}
                </p>
                <p style={{ fontSize: "13px", color: "var(--color-text-2)" }}>
                  Tel: {liveView.phone} · {liveView.email}
                </p>
              </div>
            ...
            {liveView.accountStatus !== "normal" && (
              ...
              >
                ⚠ Account overdue by{" "}
                <strong>{liveView.paymentOverdueDays ?? "?"} days</strong>.
                {liveView.accountStatus === "in_default"
            ...
            >
              {[
                {
                  label: "Credit Limit",
                  value: `£${liveView.creditLimit.toLocaleString()}`,
                },
                {
                  label: "Current Balance",
                  value: `£${liveView.currentBalance.toFixed(2)}`,
                },
                {
                  label: "Discount Plan",
                  value:
            ...
            {liveView.discountPlan.type === "flexible" && (
              <div>
                ...
                >
                  Flexible Discount Tiers (resolved end of calendar month)
                </p>
                <Table
                  columns={[
                    {
                    ...
                    },
                    {
                      ...
                      render: (r) => (
                        <span className="mono">
                          {r.maxValue ? `£${r.maxValue.toLocaleString()}` : "∞"}
                        </span>
                      ),
                    },
                    {
                  ...
                  data={liveView.discountPlan.tiers}
                  keyField="minValue"
                />
                <p
                  style={{
            ...
            >
              ...
              >
                ...
                <Field label="Reference / Note">
                  <Input
                    ...
                    placeholder="Transaction ref or cheque no."
                  />
                </Field>
              </div>
              ...
              {liveView.accountStatus === "suspended" &&
                liveView.currentBalance > 0 && (
                  ...
                  >
                    ✓ Full payment will automatically restore account to Normal
                    status.
                  </p>
                )}
            </div>
      ...
      >
        {deleteConfirm && (
          ...
          >
            ...
            >
              <p>
                <strong>Cascaded delete — permanently removes:</strong>
              </p>
              <ul
                style={{
                  marginTop: "6px",
              ...
              >
                <li>
                  Merchant: <strong>{deleteConfirm.companyName}</strong> (
                  {deleteConfirm.iposAccount})
                </li>
                <li>All associated orders and invoices</li>
                <li>
                  Login: <strong>{deleteConfirm.loginUsername}</strong>
                </li>
        ...
        footer={
          <>
            ...
            <Button onClick={handleSave}>
              {editMerchant ? "Save Changes" : "Create Account"}
            </Button>
          </>
        }
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          {/* ✅ STEP 4: Validation error banner */}
          {validationError && (
            <div
              style={{
                padding: "10px 14px",
          ...
          <div>
            ...
            >
              ...
              <Field label="Fax">
                <Input
                  value={form.fax}
                  onChange={(e) => f("fax", e.target.value)}
                />
              </Field>
            </div>
          </div>
            ...
            >
              ...
              >
                <Input
                  ...
                  onChange={(e) => f("loginPassword", e.target.value)}
                />
              </Field>
            </div>
          </div>
            ...
            {form.discountType === "fixed" && (
              <Field label="Discount Rate (%)">
                <Input
                  ...
                  onChange={(e) => f("fixedRate", e.target.value)}
                  style={{ width: "120px" }}
                />
              </Field>
            )}
                ...
                >
                  <Field label="Tier 1 max (£)">
                    <Input
                      type="number"
                      value={form.tier1Max}
                      onChange={(e) => f("tier1Max", e.target.value)}
                    />
                  </Field>
                  <Field label="Tier 1 rate (%)">
                    <Input
                      ...
                      onChange={(e) => f("tier1Rate", e.target.value)}
                    />
                  </Field>
                  <div />
                  <Field label="Tier 2 max (£)">
                    <Input
                      type="number"
                      value={form.tier2Max}
                      onChange={(e) => f("tier2Max", e.target.value)}
                    />
                  </Field>
                  <Field label="Tier 2 rate (%)">
                    <Input
                      ...
                      onChange={(e) => f("tier2Rate", e.target.value)}
                    />
                  </Field>
                  <div />
                  <Field label="Tier 3 rate (%)" hint="No upper bound">
                    <Input
                      type="number"
                      value={form.tier3Rate}
                      onChange={(e) => f("tier3Rate", e.target.value)}
                    />
                  </Field>
                </div>
              </div>
