...
export interface AppDataContextValue {
  ...
  addCatalogueItem: (
    item: Omit<CatalogueItem, "id">,
    idOverride?: string,
  ) => Promise<void>;
  updateCatalogueItem: (
    id: string,
    updates: Partial<CatalogueItem>,
  ) => Promise<void>;
...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const merchantAccountId =
    isMerchant && user?.id ? parseInt(user.id, 10) : null;
  ...
  const loadAccounts = useCallback(async () => {
    ...
    if (merchantAccountId) {
      setMerchants(
        merchantAccounts
          .filter((a) => a.accountId === merchantAccountId)
          .map(adaptAccount),
      );
    } else {
  ...
  const loadOrdersAndInvoices = useCallback(async () => {
    ...
    const [rawOrders, rawInvoices] = merchantAccountId
      ...
      : await Promise.all([ordersApi.getAll(), invoicesApi.getAll()]);
  ...
  const updateMerchant = useCallback(
    async (id: string, updates: Partial<Merchant>) => {
      ...
      if (hasContact) {
        await accountsApi.updateContact(backendId, {
          ...
          address: [updates.address, updates.city, updates.postcode]
            .filter(Boolean)
            .join(", "),
          phone: updates.phone,
          fax: updates.fax,
      ...
      if (updates.discountPlan) {
        ...
        const existingPlanId = (existing as any)?._discountPlanId as
          | number
          | undefined;
        const planId = await syncDiscountPlan(
          updates.discountPlan,
          existingPlanId,
        );
  ...
  const addCatalogueItem = useCallback(
    async (item: Omit<CatalogueItem, "id">, idOverride?: string) => {
  ...
  const updateCatalogueItem = useCallback(
    async (id: string, updates: Partial<CatalogueItem>) => {
  ...
  const placeOrder = useCallback(
    async (merchantId: string, items: OrderItem[]): Promise<Order> => {
          ...
          prev.map((o) =>
            o.id === orderId
              ? {
                  ...o,
                  status,
                  ...(dispatchDetails ? { dispatch: dispatchDetails } : {}),
                }
              : o,
          ),
        );
  ...
  const getMerchantOrders = useCallback(
    (mid: string) => orders.filter((o) => o.merchantId === mid),
    [orders],
  );
  const getMerchantInvoices = useCallback(
    (mid: string) => invoices.filter((i) => i.merchantId === mid),
    [invoices],
  );
  const getOverdueMerchants = useCallback(
    () => merchants.filter((m) => m.accountStatus !== "normal"),
    [merchants],
  );
  const getLowStockItems = useCallback(
    () => catalogue.filter((c) => c.availability < c.stockLimit),
    [catalogue],
  );
  const getIncompleteOrders = useCallback(
    () =>
      orders.filter((o) =>
        ["submitted", "accepted", "ready_to_dispatch", "dispatched"].includes(
          o.status,
        ),
      ),
    [orders],
